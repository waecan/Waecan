...
use waecan_crypto::address::WaecanAddress;
use waecan_crypto::hash::hash_to_point;
use waecan_crypto::keys::{derive_keypairs, MasterSeed};
use waecan_crypto::pedersen::PedersenCommitment;
use waecan_crypto::ring_sig::{mlsag_sign, Ring, RingMember};
use waecan_crypto::stealth::{compute_output_key, derive_output_private_key, scan_output};
...
mod tests {
    ...
    fn test_scan_block_finds_owned_output() {
        ...
        let block = Block {
            header: waecan_core::block::BlockHeader {
                version: 1,
                prev_hash: [0u8; 32],
                merkle_root: [0u8; 32],
                timestamp: 0,
            ...
            coinbase: waecan_core::block::CoinbaseTx {
                height: 0,
                reward: 0,
